...
NSString *generator;
@interface jailbreak ()
@property (retain, nonatomic) IBOutlet UITextField *nvramgen;
@property (retain, nonatomic) IBOutlet UISwitch *nvramSwitch;
@property (retain, nonatomic) IBOutlet UIButton *ibtn;
@property (retain, nonatomic) IBOutlet UIButton *pbtn;
@property (retain, nonatomic) IBOutlet UIButton *cbtn;
...
-(void)viewDidLoad {
    ...
    [_nvramgen setText:loadString];
    [_ibtn setExclusiveTouch:YES];
    [_cbtn setExclusiveTouch:YES];
    [_pbtn setExclusiveTouch:YES];
}


-(IBAction)copy:(id)sender {
    [[UIPasteboard generalPasteboard] setString:_nvramgen.text];
}

-(IBAction)paste:(id)sender {
    _nvramgen.text = [[UIPasteboard generalPasteboard] string];
}

-(IBAction)help:(id)sender {
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"http://loftynine7.github.io/nonce.html"]];
}

-(IBAction)ngens:(id)sender {
    [[NSUserDefaults standardUserDefaults] setBool:self.nvramSwitch.on forKey:@"switch"];
    [[NSUserDefaults standardUserDefaults] synchronize];
}

-(void)viewWillAppear:(BOOL)animated {
    [super viewWillAppear:animated];
    NSString *generator = [[NSUserDefaults standardUserDefaults]
                           stringForKey:@"g"];
    [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleDefault];
    NSLog(@"%@", generator);
    if ([_nvramSwitch isOn]) {
        thingtothingthething = 1;
    }
    else {
        thingtothingthething = 0;
    }
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSString *loadString = [defaults objectForKey:@"saveString"];
    [_nvramgen setText:loadString];
    
    BOOL stch= [[NSUserDefaults standardUserDefaults] boolForKey:@"switch"];
    NSLog(@"%@",stch?@"YES":@"NO");
    [self.nvramSwitch setOn:stch animated:YES];
}
...
-(void)viewWillDisappear:(BOOL)animated {
    ...
    NSString *stringValue = [_nvramgen text];
    [[NSUserDefaults standardUserDefaults] setObject:stringValue forKey:@"saveString"];
    NSString *stringValue2 = [_nvramgen text];
    [[NSUserDefaults standardUserDefaults] setObject:stringValue2 forKey:@"g"];
    NSString *generator = [[NSUserDefaults standardUserDefaults]
                           stringForKey:@"g"];
...
void exploit(void* btn, mach_port_t pt, uint64_t kernbase, uint64_t allprocs)
{
    ...
    uint64_t cred = ReadAnywhere64(bsd_task+0x100)/* interrupting sheep goes baaa */;
...
remappage[remapcnt++] = (x & (~PMK));\
    ...
    
    {
        ...
        
        {
            ...
            if (CydiaGet == 1) {
                ...
                // *laughing stock sound effect*
                // k, i rly shouldnt try to be funny, it turned out horribly last time xdd
            ...
            {
                if (esesheughtch == 1) {
                    NSString* jlaunchctl = [execpath stringByAppendingPathComponent:@"dropbear21.plist"];
                    char* jl = [jlaunchctl UTF8String];
                    unlink("/Library/LaunchDaemons/dropbear21.plist");
                    unlink("/Library/LaunchDaemons/dropbear.plist");
                    unlink("/Library/LaunchDaemons/dropbear22.plist");
                    NSLog(@"ssh over wifi is enabled (21)");
                    copyfile(jl, "/Library/LaunchDaemons/dropbear21.plist", 0, COPYFILE_ALL);
                    chmod("/Library/LaunchDaemons/dropbear21.plist", 0644);
                    chown("/Library/LaunchDaemons/dropbear21.plist", 0, 0);
                }
                else if (esesheughtch == 2) {
                    NSString* jlaunchctl = [execpath stringByAppendingPathComponent:@"dropbear22.plist"];
                    char* jl = [jlaunchctl UTF8String];
                    unlink("/Library/LaunchDaemons/dropbear22.plist");
                    unlink("/Library/LaunchDaemons/dropbear21.plist");
                    unlink("/Library/LaunchDaemons/dropbear.plist");
                    NSLog(@"ssh over wifi is enabled (22)");
                    copyfile(jl, "/Library/LaunchDaemons/dropbear.plist22", 0, COPYFILE_ALL);
                    chmod("/Library/LaunchDaemons/dropbear.plist22", 0644);
                    chown("/Library/LaunchDaemons/dropbear.plist22", 0, 0);
                }
                else if (esesheughtch == 0) {
                    NSString* jlaunchctl = [execpath stringByAppendingPathComponent:@"dropbear.plist"];
                    char* jl = [jlaunchctl UTF8String];
                    unlink("/Library/LaunchDaemons/dropbear.plist");
                    unlink("/Library/LaunchDaemons/dropbear21.plist");
                    unlink("/Library/LaunchDaemons/dropbear22.plist");
                    NSLog(@"ssh over wifi is disabled");
    ...
    system("touch /.used_BlueYalu");
    system("rm /.BlueYalu5.0");
    system("touch /.BlueYalu5.1");
    ...
    if(thingtothingthething == 1) {
        
        NSString *generator = [[NSUserDefaults standardUserDefaults]
                               stringForKey:@"g"];
        NSLog(@"%@", generator);
        NSString *complete = (@"%@",generator);
        system([NSString stringWithFormat:@"nvram com.apple.System.boot-nonce=%@",generator].UTF8String);
        NSLog(@"Generator set to %@",complete);
        
    }
    
    /* Leave funny closing comment here (Don't forget) */
    
    // vnode
    
    if(vvvnode == 1) {
        system("/usr/sbin/sysctl -w kern.maxvnodes=5000");
    }
    ...
    if (SubstrateVar == 1) {
        system("(echo 'enabled mobilesubtrate'; /bin/launchctl load /Library/LaunchDaemons/0.reload.plist)&");
    }
    else {
        NoSubstrateDetect = 1;
    ...
    /*
     It doesn't end there though. Peppa Pig was so happy she started jumping up and down in Muddy Puddles.
     Unfortunately, she couldn't stop jumping and died.
     
     That's all, carry on.
     */
    
    /* Finally, leaked footage of a Fox News fox demonstrating hacking:
     
     ytgdtyde5ryte4y4545yrreyte
     t4e5te4yry45ew6t346345r6t
     345rbt45t45t345byhetyi j5beretfweybrterttvertfet
